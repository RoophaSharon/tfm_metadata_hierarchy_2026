...
st.title(methods.title('Approach 1'))
st.caption('Upload a metadata file (CSV / TSV / XLSX / JSON), confirm the column '
           'roles, then build. External enrichment activates automatically for '
...
def _cluster_and_label(tdf, path_prefix, nodes, leaf_to_id, embedder,
    ...
    for k in range(n_clust):
        ...
        # singleton%/n_agg and moved the tree away from the manual-reference subset. Castanet's Condition facet
...
def build_concept_hierarchy(can, embedder, concept_table, project='metadata_project',
    ...
    # it regressed setOverlap (0.914→0.836: mis-grouping) and added depth beyond the reference subset.
...
with st.expander('Advanced settings', expanded=False):
    ...
    existing = st.file_uploader('Load existing hierarchy JSON instead', type=['json'])
...
with tabs[8]:
    ...
    _out_dir = Path(__file__).resolve().parent.parent / 'outputs' / 'approach_1'
...
with tabs[10]:
    ...
    if can_eval.empty or not nodes_eval:
        ...
        st.caption(
            ...
            'comparable across methods as an accuracy metric.'
        )
